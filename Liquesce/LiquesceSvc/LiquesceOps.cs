...
namespace LiquesceSvc
{
    internal class LiquesceOps : IDokanOperations
    {
        ...
        public LiquesceOps(ConfigDetails configDetails)
        {
            ...
            this.filemanager = new FileManager(configDetails, roots);
        ...
        public int MoveFile(string filename, string newname, bool replaceIfExisting, DokanFileInfo info)
        {
            try
            {
                ...
                else
                {
                    ...
                    List<string> paths1 = roots.GetAllPaths(filename);
                    if (paths1.Count == 0)
                    {
                    ...
                    for (int i = 0; i < paths1.Count; i++)
                    {
                        ...
                        string path2 = Roots.GetRoot(path1) + newname;
                        filemanager.XMoveDirectory(path1, path2, replaceIfExisting);
                    }
                }
            }
            catch (Exception ex)
